#include<bits/stdc++.h>
using namespace std;
//想法 -> bfs -> 每個點都做運算 並賦值(上一層的運算結果) 用dp儲存-> 在bfs遍歷下一層(push到queue之前)時 將下一層的path設為上一層+1
int main(){
    ...
    vector<vector<int>> map(p+q+r+1),dp(p+q+r+1); //dp紀錄每個點的狀態 *邏輯閘->dp[邏輯閘種類,訊號1,訊號2] *輸入端口->dp[5,訊號] *輸出端口->dp[訊號]
    queue<int> que;
    for (int i=0; i<p; i++){
        cin >> a;
        in.push_back(a);
    ...
    while(!que.empty()){
        ...
        if (!map[node].empty()){
            if (node>p){//判斷不是輸入端口做計算 遍歷子節點 並把計算結果push到子節點
                ...
                dp[map[node][0]].push_back(5);//之後可以判斷輸入端口的下一層 
